...
func TestVersionFlag(t *testing.T) {
	for _, args := range [][]string{{"--version"}, {"-v"}, {"version"}} {
		out, err := run(t, args...)
		if err != nil || !strings.HasPrefix(out, "dpilot version ") {
			t.Fatalf("%v: err=%v out=%q", args, err, out)
		}
	}
	for _, args := range [][]string{{"-j", "--version"}, {"-j", "version"}} {
		out, err := run(t, args...)
		if err != nil {
			t.Fatal(err)
		}
		...
		if json.Unmarshal(e.Raw, &raw) != nil || raw["version"] == "" || !strings.HasPrefix(e.Msg, "dpilot version ") {
			t.Fatalf("%v: version must honour -j like ddev, got %q", args, out)
		}
	}
}

func TestLifecycleAcceptsSeveralGroups(t *testing.T) {
	t.Setenv("DPILOT_HOME", t.TempDir())
	...
	if _, err := run(t, "start", "b", "a"); err != nil {
		t.Fatalf("start b a: %v", err)
	}
	if strings.Join(rec.started, ",") != "y,x" {
		t.Fatalf("groups run in the order given, got %v", rec.started)
	}
}

func TestRestartAllStopsEverythingBeforeStarting(t *testing.T) {
	t.Setenv("DPILOT_HOME", t.TempDir())
	_ = config.Save(&config.Group{Name: "a", Members: []string{"x"}})
	_ = config.Save(&config.Group{Name: "b", Members: []string{"y"}})
	rec := &recClient{}
	newClient = func() ddev.Client { return rec }
	if _, err := run(t, "restart", "--all"); err != nil {
		t.Fatalf("restart --all: %v", err)
	}
	if got := strings.Join(rec.calls, ","); got != "stop:y,stop:x,start:x,start:y" {
		t.Fatalf("restart --all must be stop --all then start --all, got %q", got)
	}
}
